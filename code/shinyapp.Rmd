...
generateBarPlot <- function(data, variable) {
  variable <- enquo(variable)  # Enquoting the variable
  
  # Group by the specified variable and calculate average stars
  plot_data <- data %>%
    group_by(!!variable) %>%
    filter((!(!!variable==""))&(!is.na(!!variable))&(!(!!variable=="None"))) %>%
    summarise(avg_star = mean(stars), .groups = "drop")
  
  # Create the bar plot
  plot <- ggplot(plot_data, aes(x = !!variable, y = avg_star)) +
    geom_bar(stat = "identity", fill = "skyblue", color = "black", alpha = 0.9) +
    labs(x = glue("{rlang::quo_name(variable)} Category"), y = "Average Stars") +
    geom_text(aes(label = sprintf("%.2f", avg_star)), vjust = -0.5, size = 3) +
    ggtitle(glue("Average Stars by {rlang::quo_name(variable)} Category")) +
    diytheme
...
server <- function(input, output, session) {
  ...
  output$barPlot <- renderPlot({
    variable <- as.name(input$barPlotVariable)
    generateBarPlot(joined_data, variable)
  })
}
